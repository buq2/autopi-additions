...
class FakedNet:
    ...
    def get_fake_network_interface_state(self):
        out = {}
        out[self.name] = {my_network.RECEIVED:self.received, my_network.TRANSMITTED:self.transmitted}
    ...
    def reset(self):
        ...
        self.transmitted = np.min([np.random.randint(0,10), self.transmitted-1])
...
@pytest.fixture
def net():
    my_network.MIN_DIFFERENCE_BETWEEN_TIMESERIES_POINTS = datetime.timedelta(seconds=0)
    my_network.clear()
    net = FakedNet()
    my_network.__get_network_interface_state = lambda: net.get_fake_network_interface_state()
    return net
